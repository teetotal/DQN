...
class DQN:
    ...
    def __init__(self, session, width, global_step):
        ...
        self.input_X = tf.placeholder(tf.float32, [None, width])
    ...
    def _build_network(self, name):
        with tf.variable_scope(name):
            ...
            '''
            model = tf.layers.dense(self.input_X, numNN, activation=tf.nn.relu)
            model = tf.layers.dense(model, numNN, activation=tf.nn.relu)
            model = tf.layers.dense(model, numNN, activation=tf.nn.relu)
            model = tf.layers.dense(model, numNN, activation=tf.nn.relu)

            Q = tf.layers.dense(model, 1, activation=None)
    ...
    def _build_op(self, global_step):
        ...
        #reduce_mean = 평균 
        #reduce_sum = 합
        one_hot = tf.reshape(self.input_A, [-1,1])
        Q_value = tf.reduce_sum(tf.multiply(self.Q, one_hot), axis=1)
        cost = tf.reduce_mean(tf.square(self.input_Y - Q_value))
    ...
    '''
    def remember(self, state, next_state, action, reward, terminal):
        
        # 플레이결과, 즉, 액션으로 얻어진 상태와 보상등을 메모리에 저장합니다.
        self.memory.append((state, next_state, action, reward, terminal))
    ...
    def train(self):
        ...
        #Y = [None for _ in range(self.BATCH_SIZE)]
        Y = []
        for i in range(self.BATCH_SIZE):
            if terminal[i]:
                #Y.append(reward[i])
                #print("Rank", tf.rank(reward[i]))
                Y = np.append(Y, reward[i])
            else:
                #Y.append(reward[i] + self.GAMMA * np.max(target_Q_value[i]))
                #print("Rank reward[i]", tf.rank(reward[i]), reward[i] + self.GAMMA * np.max(target_Q_value[i]))
                Y = np.append(Y, reward[i] + self.GAMMA * np.max(target_Q_value[i]))

        #print(Y)
        Y = tf.constant(Y, shape=[self.BATCH_SIZE])
        #Y_val = self.session.run(Y)
        Y = tf.reshape(Y, shape=[32])
        #print(Y_val)
        self.session.run(self.train_op,
                         feed_dict={
                             self.input_X: state,
                             self.input_A: action,
                             self.input_Y: self.session.run(Y)
                         })