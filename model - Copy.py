...
class DQN:
    ...
    def _build_network(self, name):
        with tf.variable_scope(name):
            model = tf.layers.conv2d(self.input_X, 64, [2, 2], padding='same', activation=tf.nn.relu)
            model = tf.layers.conv2d(model, 128, [1, 1], padding='same', activation=tf.nn.relu)
            #model = tf.layers.conv2d(model, 64, [1, 1], padding='same', activation=tf.nn.relu)
