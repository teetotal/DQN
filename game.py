...
class Game:
    ...
    def _prepare_display(self):
        ...
        # 키입력 추가
        fig.canvas.mpl_connect('key_press_event', self._keyPressed)
    ...
    def _draw_screen(self):
        ...
        block1 = patches.RegularPolygon((self.block[0]["col"], self.block[0]["row"]),
                                    6,
                                    0.5,
                                    linewidth=0, facecolor="#00FF00")
        block2 = patches.RegularPolygon((self.block[1]["col"], self.block[1]["row"]),
                                    6,
                                    0.5,
                                    linewidth=0, facecolor="#AFFF00")
        #if self.show_game:
        if (self.car["col"], self.car["row"]) == (self.block[0]["col"], self.block[0]["row"]) or (self.car["col"], self.car["row"]) == (self.block[1]["col"], self.block[1]["row"]):
            car = patches.RegularPolygon((self.car["col"], self.car["row"]), 3, 0.5, linewidth=0, facecolor="#FF0000")
            self.axis.set_title("Failure!!", fontsize=10)
        else:
            car = patches.RegularPolygon((self.car["col"], self.car["row"]), 3, 0.5, linewidth=0, facecolor="#1111FF")
